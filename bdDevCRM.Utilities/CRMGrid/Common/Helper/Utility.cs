...
//namespace Utilities.Common.Helper
//{
...
//            return
//                (from type in asm.GetTypes() where type.FullName.Equals(typeName) select Activator.CreateInstance(type))
//                    .FirstOrDefault();
//        }
...
//            {
//                var appRootPath = AppDomain.CurrentDomain.SetupInformation.PrivateBinPath ??
...
//            if (!File.Exists(path + type_assem[1]))
//                throw new Exception("Cant find " + fullQualifidName + " in " + path + type_assem[1]);
//            var asm = Assembly.LoadFrom(path + type_assem[1]);
//            return
//                (from type in asm.GetTypes()
//                    where type.FullName.Equals(type_assem[0])
//                    select Activator.CreateInstance(type)).FirstOrDefault();
//        }
