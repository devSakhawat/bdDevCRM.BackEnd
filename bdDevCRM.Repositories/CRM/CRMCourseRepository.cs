...
public class CRMCourseRepository : RepositoryBase<Crmcourse>, ICRMCourseRepository
{
  ...
  public async Task<IEnumerable<Crmcourse>> GetActiveCourseAsync(bool trackChanges) =>
      await ListByConditionAsync(x => x.Status == true, c => c.InstituteId, trackChanges, descending: false);
  ...
  ////public override async Task<CRMCourse?> ExecuteSingleSql(string query) =>
