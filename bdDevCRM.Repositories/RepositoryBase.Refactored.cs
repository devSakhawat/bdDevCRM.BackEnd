...
public class RepositoryBaseRefactored<T> : IRepositoryBase<T> where T : class
{
    ...
    /// <returns>Entity or null if not found</returns>
    /// <exception cref="ArgumentNullException">Thrown when predicate is null</exception>
    public async Task<T?> GetByIdAsync(Expression<Func<T, bool>> predicate, bool trackChanges = false, CancellationToken cancellationToken = default)
    {
        if (predicate == null)
            throw new ArgumentNullException(nameof(predicate));
    ...
    /// <returns>Collection of entities</returns>
    /// <exception cref="ArgumentNullException">Thrown when expression is null</exception>
    public async Task<IEnumerable<T>> GetListByIdsAsync(Expression<Func<T, bool>> expression, bool trackChanges = false, CancellationToken cancellationToken = default)
    {
        if (expression == null)
            throw new ArgumentNullException(nameof(expression));
    ...
    /// <returns>Collection of entities</returns>
    /// <exception cref="ArgumentException">Thrown when query is null or whitespace</exception>
    public async Task<IEnumerable<T>?> ExecuteListSql(string query, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(query))
            throw new ArgumentException("Query cannot be null or empty.", nameof(query));
    ...
    /// <exception cref="ArgumentException">Thrown when query is null or whitespace</exception>
    public async Task<T?> ExecuteSingleSql(string query, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(query))
            throw new ArgumentException("Query cannot be null or empty.", nameof(query));
    ...
    public async Task<GridEntity<TGrid>> GridData<TGrid>(string query, CRMGridOptions options, string orderBy, string condition, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(query))
            throw new ArgumentException("Query cannot be null or empty.", nameof(query));
    ...
    public async Task<TResult?> ExecuteSingleData<TResult>(string query, SqlParameter[]? parameters = null, CancellationToken cancellationToken = default) where TResult : class, new()
    {
        if (string.IsNullOrWhiteSpace(query))
            throw new ArgumentException("Query cannot be null or empty.", nameof(query));
        ...
        try
        {
            ...
            if (await reader.ReadAsync(cancellationToken))
            {
                ...
                result = MapReaderToObject(reader, propertyMap);
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<TResult>> ExecuteListQuery<TResult>(string query, SqlParameter[]? parameters = null, CancellationToken cancellationToken = default) where TResult : class, new()
    {
        if (string.IsNullOrWhiteSpace(query))
            throw new ArgumentException("Query cannot be null or empty.", nameof(query));
        ...
        try
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                results.Add(MapReaderToObject(reader, propertyMap));
            }
