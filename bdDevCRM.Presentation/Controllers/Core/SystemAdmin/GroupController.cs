...
public class GroupController : BaseApiController
{
	...
	public async Task<IActionResult> GroupSummary([FromBody] CRMGridOptions options)
	{
		...
		if (groupSummary == null || !groupSummary.Items.Any())
			return Ok(ApiResponseHelper.NoContent<GridEntity<GroupSummaryDto>>("No groups found"));
	...
	public async Task<IActionResult> GroupPermisionsbyGroupId([FromRoute] int groupId)
	{
		...
		// Return standardized response
		if (res == null || !res.Any())
			return Ok(ApiResponseHelper.NoContent<IEnumerable<GroupPermissionDto>>(
	...
	public async Task<IActionResult> GetAccesses()
	{
		...
		if (accessControls == null || !accessControls.Any())
			return Ok(ApiResponseHelper.NoContent<IEnumerable<AccessControlDto>>(
					"No access controls found"));

		return Ok(ApiResponseHelper.Success(accessControls,
	...
	public async Task<IActionResult> SaveGroup([FromBody] GroupDto objGroup)
	{
		...
		return Ok(ApiResponseHelper.Created(model,
	...
	public async Task<IActionResult> UpdateGroup([FromRoute] int groupId, [FromBody] GroupDto modelDto)
	{
		...
		return Ok(ApiResponseHelper.Updated(returnData, "Group updated successfully"));
	}
	...
	public async Task<IActionResult> GetGroups()
	{
		...
		if (groupForUserSettings == null || !groupForUserSettings.Any())
			return Ok(ApiResponseHelper.NoContent<IEnumerable<GroupForUserSettings>>(
					"No groups found"));

		return Ok(ApiResponseHelper.Success(groupForUserSettings.ToList(),
	...
	public async Task<IActionResult> GroupMemberByUserId([FromQuery] int userId)
	{
		...
		// Return standardized response
		if (groupForUserSettings == null || !groupForUserSettings.Any())
			return Ok(ApiResponseHelper.NoContent<IEnumerable<GroupMemberDto>>(
	...
	public async Task<IActionResult> GetAccessPermisionForCurrentUser([FromBody] CommonProperty model)
	{
		...
		if (groupPermissions == null || !groupPermissions.Any())
			return Ok(ApiResponseHelper.NoContent<IEnumerable<GroupPermissionDto>>("No group permissions found"));

		return Ok(ApiResponseHelper.Success(groupPermissions.ToList(), "Group permissions retrieved successfully"));
	}
}