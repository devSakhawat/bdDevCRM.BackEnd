...
internal sealed class ApplicantCourseService(
    IRepositoryManager repository,
    ILoggerManager logger,
    IConfiguration config,
    IHttpContextAccessor httpContextAccessor) : IApplicantCourseService
{
  ...
  {
    var list = await _repository.ApplicantCourse.GetActiveApplicantCoursesAsync(trackChanges);
    if (!list.Any()) throw new GenericListNotFoundException("ApplicantCourse");
    return MyMapper.JsonCloneIEnumerableToList<ApplicantCourse, ApplicantCourseDto>(list);
  }
  ...
  {
    var list = await _repository.ApplicantCourse.GetApplicantCoursesAsync(trackChanges);
    if (!list.Any()) throw new GenericListNotFoundException("ApplicantCourse");
    return MyMapper.JsonCloneIEnumerableToList<ApplicantCourse, ApplicantCourseDto>(list);
  }
  ...
  {
    var entity = await _repository.ApplicantCourse.GetApplicantCourseAsync(id, trackChanges);
    if (entity == null) throw new GenericNotFoundException("ApplicantCourse", "ApplicantCourseId", id.ToString());
    return MyMapper.JsonClone<ApplicantCourse, ApplicantCourseDto>(entity);
  }
  ...
  {
    var list = await _repository.ApplicantCourse.GetApplicantCoursesByApplicantIdAsync(applicantId, trackChanges);
  ...
  public async Task<ApplicantCourseDto> CreateNewRecordAsync(ApplicantCourseDto dto, UsersDto currentUser)
  {
    ...
    bool dup = await _repository.ApplicantCourse.ExistsAsync(x => x.ApplicantId == dto.ApplicantId && x.CourseId == dto.CourseId);
    if (dup) throw new DuplicateRecordException("ApplicantCourse", "ApplicantId-CourseId combination");

    var entity = MyMapper.JsonClone<ApplicantCourseDto, ApplicantCourse>(dto);
    entity.CreatedDate = DateTime.UtcNow;
    entity.CreatedBy = currentUser.UserId ?? 0;
    
    dto.ApplicantCourseId = await _repository.ApplicantCourse.CreateAndGetIdAsync(entity);
  ...
  public async Task<string> UpdateRecordAsync(int key, ApplicantCourseDto dto, bool trackChanges)
  {
    ...
    bool exists = await _repository.ApplicantCourse.ExistsAsync(x => x.ApplicantCourseId == key);
    if (!exists) throw new GenericNotFoundException("ApplicantCourse", "ApplicantCourseId", key.ToString());

    var entity = MyMapper.JsonClone<ApplicantCourseDto, ApplicantCourse>(dto);
    entity.UpdatedDate = DateTime.UtcNow;
    
    _repository.ApplicantCourse.Update(entity);
  ...
  public async Task<string> DeleteRecordAsync(int key, ApplicantCourseDto dto)
  {
    ...
    await _repository.ApplicantCourse.DeleteAsync(x => x.ApplicantCourseId == key, true);
    await _repository.SaveAsync();
  ...
  {
    var entity = await _repository.ApplicantCourse.GetApplicantCourseByApplicantAndCourseIdAsync(applicantId, courseId, trackChanges);
...
";
    ...
    return await _repository.ApplicantCourse.GridData<ApplicantCourseDto>(sql, options, orderBy, "");
  }
}