// =========================================================================================================
// 📄 File: CrmApplicationService.WithCancellationToken.Example.cs
// 📝 Description: CancellationToken implementation উদাহরণ - Service Layer
// 🎯 Purpose: Existing CrmApplicationService কে CancellationToken support সহ refactor করার উদাহরণ
//
// ⚠️ NOTE: এটি একটি EXAMPLE FILE। Production code এ ব্যবহার করার আগে:
//    1. Original CrmApplicationService.cs এ implement করুন
//    2. সব dependencies সঠিকভাবে inject করুন
//    3. Interface update করুন (ICrmApplicationService)
//    4. Proper testing করুন
//    5. এই example file delete করুন
// =========================================================================================================
...
namespace bdDevCRM.Service.CRM.Examples;
...
public class CrmApplicationServiceWithCancellationTokenExample : ICrmApplicationService
{
      ...
      CancellationToken cancellationToken = default) // ✅ Added with default value
  {
    try
    {
      ...
      IEnumerable<GroupPermissionRepositoryDto> returnResult2 =
          await _repository.Groups.GetAccessPermisionForCurrentUser(
              menuDto.ModuleId.Value,
              usersDto.UserId.Value,
              cancellationToken);
      ...
      IEnumerable<GroupPermissionDto> result2 =
          MyMapper.JsonCloneIEnumerableToIEnumerable<GroupPermissionRepositoryDto, GroupPermissionDto>(returnResult2);
  ...
  /* ========================================
     ✅ EXAMPLE 2: GetApplication with Multiple Queries
      ...
      CancellationToken cancellationToken = default) // ✅ Added with default value
  {
    try
    {
      ...
      if (additionalDocuments != null && additionalDocuments.Any())
      {
        result.AdditionalDocuments = MyMapper.JsonCloneIEnumerableToIEnumerable<
            AdditionalDocumentRepositoryDto,
    ...
    {
      _logger.LogInformation(
          "GetApplication operation cancelled for ApplicationId: {ApplicationId}",
          applicationId);
      throw;
    }
    catch (Exception ex)
    {
      _logger.LogError(ex, "Error in GetApplication for ApplicationId: {ApplicationId}", applicationId);
      throw;
    }
  }
      ...
      CancellationToken cancellationToken = default) // ✅ Added with default value
  {
    ...
    try
    {
      ...
      // ✅ Pass CancellationToken to CreateAndGetIdAsync
      int applicationId = await _repository.CrmApplications.CreateAndGetIdAsync(
      ...
      if (dto.CourseInformation?.PersonalDetails != null)
      {
        ...
        int applicantId = await _repository.CrmApplicantInfoes.CreateAndGetIdAsync(
            applicantInfoEntity,
            cancellationToken);
        ...
        if (dto.CourseInformation?.ApplicantCourse != null)
        {
          ...
          // ✅ Pass CancellationToken
          var applicantCourseId = await _repository.CrmApplicantCourses.CreateAndGetIdAsync(
        ...
        if (dto.CourseInformation?.ApplicantAddress?.PermanentAddress != null)
        {
          ...
          // ✅ Pass CancellationToken
          var permanentAddressId = await _repository.CrmPermanentAddresses.CreateAndGetIdAsync(
        ...
        if (dto.CourseInformation?.ApplicantAddress?.PresentAddress != null)
        {
          ...
          // ✅ Pass CancellationToken
          var presentAddressId = await _repository.CrmPresentAddresses.CreateAndGetIdAsync(
            ...
            dto.EducationInformation.EducationDetails.EducationHistory.Any())
        {
          foreach (var educationDto in dto.EducationInformation.EducationDetails.EducationHistory)
          {
            ...
            // ✅ Pass CancellationToken
            var educationHistoryId = await _repository.CrmEducationHistories.CreateAndGetIdAsync(
                educationEntity,
                cancellationToken);
          }
        }
        ...
        if (dto.EducationInformation?.IELTSInformation != null)
        {
          ...
          // ✅ Pass CancellationToken
          var ieltsEntityId = await _repository.CrmIELTSInformations.CreateAndGetIdAsync(
              ieltsEntity,
              cancellationToken);
        }
      ...
      CancellationToken cancellationToken = default) // ✅ Added with default value
  {
    ...
    if (key != updateDto.ApplicationId)
      throw new IdMismatchBadRequestException(key.ToString(), "Application Update");
    ...
    if (!exists)
      throw new GenericNotFoundException("Application", "ApplicationId", key.ToString());
      ...
      if (updateDto.CourseInformation?.PersonalDetails != null)
      {
        ...
        if (!applicantExists)
        {
          ...
          updateDto.CourseInformation.PersonalDetails.ApplicantId =
              await _repository.CrmApplicantInfoes.CreateAndGetIdAsync(
                  crmApplicantInfo,
                  cancellationToken);
...
/* ========================================
   📝 IMPLEMENTATION CHECKLIST - Service Layer

   Service Layer এ CancellationToken implement করার জন্য:

   ✅ 1. Interface update করুন:
        Task<Result> MethodAsync(..., CancellationToken cancellationToken = default);

   ✅ 2. সব async methods এ parameter যোগ করুন with default value

   ✅ 3. Method এর শুরুতে cancellation check:
        cancellationToken.ThrowIfCancellationRequested();

   ✅ 4. সব repository calls এ token pass করুন:
        await _repository.Entity.MethodAsync(..., cancellationToken);

   ✅ 5. Multiple operations এর মধ্যে check করুন

   ✅ 6. Loop operations এ প্রতিটি iteration এ check করুন:
   ...
   ✅ 7. OperationCanceledException properly handle করুন:
        catch (OperationCanceledException)
        {
   ...
   ✅ 8. Transaction operations এ token pass করুন:
        await _repository.SaveAsync(cancellationToken);

   ⚠️ 9. Original service এ implement করার পরে এই example file delete করুন

   ⚠️ 10. Interface file (ICrmApplicationService) update করতে ভুলবেন না
...
/* ========================================
   ...
   ICrmApplicationService.cs তে এই changes করতে হবে:
   ...
   public interface ICrmApplicationService
   {
       ...
       Task<CrmApplicationDto> CreateNewRecordAsync(
           CrmApplicationDto dto,
           UsersDto currentUser,
           CancellationToken cancellationToken = default); // ✅ Add this

       Task<CrmApplicationDto> UpdateCrmApplicationAsync(
           int key,
           CrmApplicationDto updateDto,
           UsersDto currentUser,
           CancellationToken cancellationToken = default); // ✅ Add this
