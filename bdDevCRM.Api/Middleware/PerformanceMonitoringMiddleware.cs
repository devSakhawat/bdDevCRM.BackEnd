...
/// 
/// ✅ Conflict #2 resolved: No own Stopwatch — uses PipelineContext's Stopwatch
/// ✅ Conflict #5 resolved: Sets response header using OnStarting()
/// </summary>
public class PerformanceMonitoringMiddleware
{
      ...
      IConfiguration configuration)
  {
    ...
    _verySlowRequestThresholdMs = configuration.GetValue<int>(
        "PerformanceMonitoring:VerySlowRequestThresholdMs", 5000);
  }
  ...
  public async Task InvokeAsync(HttpContext context)
  {
    ...
    try
    {
      ...
      {
        _logger.LogWarning(
            "VERY SLOW REQUEST: {Method} {Path} took {Duration}ms (Status: {StatusCode}) | CorrelationId: {CorrelationId}",
